...
# Source: gu-st.ru (Mintsifry's own hosting), fetched at build time — the certs
# are NOT committed to the repo. Each download is pinned to its sha256 (DER
# form): a rotated/compromised cert on gu-st.ru fails the build instead of
# silently installing a foreign root. On a legitimate CA rotation bump the
# pins below (a root-CA rotation is exactly the kind of event worth noticing).
